...
Calvin can enter to and exit from any segment. Let's call the entry segment as index i and the exit segment as index j. Assume that the exit segment lies after the entry segment (i≤j) and 0≤i. Calvin has to pass through all segments from index ii to index j (both inclusive).
...
public class ServiceLane {
    private static int minWidth(int[] width, int i, int j){
        int min = 3;
        for(int k=i; k<=j; k++){
            if(width[k] < min){
                min = width[k];
    ...
    public static void main(String[] args) {
        ...
        for(int k = 0; k < t; k++){
            int i = in.nextInt();
            int j = in.nextInt();
            System.out.println(minWidth(width,i,j));
        }
        in.close();
    }
}